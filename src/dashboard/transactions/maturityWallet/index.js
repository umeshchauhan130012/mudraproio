...
import { transactionsMaturityWalletReport, transactionsIncomeAndWalletSummery } from "../../../redux/actions/transaction";
import { useDispatch, useSelector } from "react-redux";
import CircularProgress from "@mui/material/CircularProgress";
import { DataGrid } from "@mui/x-data-grid";

const MaturityWallet = () => {
  ...
  useEffect(() => {
    let apiData = {};
    dispatch(transactionsMaturityWalletReport(apiData));
  }, []);
  ...
  useEffect(() => {
    if (status.maturityWalletReport.data !== "") {
      if (status.maturityWalletReport.data.data.statusCode === "200") {
        setNtcl(status.maturityWalletReport.data.data.result);
        setFilterData(status.maturityWalletReport.data.data.result);
        setLoader(false);
        return;
      }
    }
    if (status.maturityWalletReport.data !== "") {
      if (status.maturityWalletReport.data.data.statusCode === "500") {
        ...
        <div className={`tableholder ${styles.bordercolor}`}>
          <div className={`${styles.tableheadertop} ${styles.titlefilter} `}>
            <div className={` ${styles.tableheadertop} ${styles.titlefiltertitle} `}>
              Total Maturity Wallet &nbsp;<label>{totalIncome && totalIncome.balanceMaturityWallte }</label> <span>USDT</span>
            </div>
