...
import { transactionsNTCLWalletReport, transactionsIncomeAndWalletSummery } from "../../../redux/actions/transaction";
...
const NTCL = () => {
  ...
  const [ntcl, setNtcl] = useState("");
  const [filterData, setFilterData] = useState(ntcl);
  ...
  useEffect(() => {
    let apiData = {};
    dispatch(transactionsNTCLWalletReport(apiData));
  }, []);
  ...
  useEffect(() => {
    if (status.NTCLWalletReport.data !== "") {
      if (status.NTCLWalletReport.data.data.statusCode === "200") {
        setNtcl(status.NTCLWalletReport.data.data.result);
        setFilterData(status.NTCLWalletReport.data.data.result);
        setLoader(false);
        return;
      }
    }
    if (status.NTCLWalletReport.data !== "") {
      if (status.NTCLWalletReport.data.data.statusCode === "500") {
    ...
    },
    {
      ...
      renderCell: (status) => {
        if (status.row.transactionType === "C") {
          ...
          <label><em className={` ${styles.lightgreen}`}>{status.row.credit} </em>MDR</label>
          <label><span>Paid - {status.row.payid}</span></label>
          </div>
          ...
          return <div className={` ${styles.titlewidthtabl} ${styles.dflexalitmcenter} `}>
          <div className={styles.titlelabelblock}>
          <label><em className={` ${styles.lightgreen} ${styles.lightredcolor}`}>{status.row.debit} </em>MDR</label>
          <label><span>Paid - {status.row.payid}</span></label>
          </div>
        ...
        <div className={`tableholder ${styles.bordercolor}`}>
          <div className={`${styles.tableheadertop} ${styles.titlefilter} `}>
            <div className={` ${styles.tableheadertop} ${styles.titlefiltertitle} `}>
              Total NTCL Income &nbsp;<label>{totalIncome && totalIncome.balanceNtclWallet }</label> <span>MDR</span>
            </div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const filteredData = filterData.filter((entry) =>
                      ...
                      entry.payid.toString().toLowerCase().includes(currValue.toString().toLowerCase())
                    );
                    setNtcl(filteredData);
                  }}
          ...
          <div className="tableresponsive hideheader" style={{position: "relative"}}>
            ...
            {ntcl.length === 0 ? <div className="alertmesstable">No Records found</div> : '' }
            {ntcl && ntcl ? 
            <DataGrid
