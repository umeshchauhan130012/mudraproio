...
import { transactionsDirectIncomeReport, transactionsIncomeAndWalletSummery } from '../../../redux/actions/transaction';
...
const DirectIncome = () => {
    ...
    const [filterData, setFilterData] = useState(directIncome);
    ...
    useEffect(() => {
      let apiData = {};
      dispatch(transactionsDirectIncomeReport(apiData));
    }, []);
    ...
    useEffect(() => {
      if (status.directIncomeReport.data !== "") {
        if (status.directIncomeReport.data.data.statusCode === "200") {
          setDirectIncome(status.directIncomeReport.data.data.result);
          setFilterData(status.directIncomeReport.data.data.result);
          setLoader(false);
          return;
        }
      }
      if (status.directIncomeReport.data !== "") {
        if (status.directIncomeReport.data.data.statusCode === "500") {
    ...
    <div className={`tableholder ${styles.bordercolor}`}>
      <div className={`${styles.tableheadertop} ${styles.titlefilter} `}>
        <div className={` ${styles.tableheadertop} ${styles.titlefiltertitle} `}>
          Total Direct Income &nbsp;<label>{totalIncome && totalIncome.totalDirectIncome}</label>{" "}
          <span>MDR</span>
        </div>
      ...
      <div className="tableresponsive hideheader" style={{position: "relative"}}>
          ...
          {directIncome.length === 0 ? <div className="alertmesstable">No Records found</div> : '' }
