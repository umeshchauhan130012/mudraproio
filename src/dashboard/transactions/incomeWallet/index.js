...
import { transactionsIncomeWalletReport, transactionsIncomeAndWalletSummery } from "../../../redux/actions/transaction";
import { DataGrid } from "@mui/x-data-grid";

const IncomeWallet = () => {
  const [incomeWallet, setIncomeWallet] = useState("");
  const [filterData, setFilterData] = useState(incomeWallet);
  ...
  useEffect(() => {
    let apiData = {};
    dispatch(transactionsIncomeWalletReport(apiData));
  }, []);
  ...
  useEffect(() => {
    if (status.incomeWalletReport.data !== "") {
      if (status.incomeWalletReport.data.data.statusCode === "200") {
        setIncomeWallet(status.incomeWalletReport.data.data.result);
        setFilterData(status.incomeWalletReport.data.data.result);
        setLoader(false);
        return;
      }
    }
    ...
    if (status.incomeWalletReport.data !== "") {
      if (status.incomeWalletReport.data.data.statusCode === "500") {
        ...
        <div className={`tableholder ${styles.bordercolor}`}>
          <div className={`${styles.tableheadertop} ${styles.titlefilter} `}>
            <div className={` ${styles.tableheadertop} ${styles.titlefiltertitle} `}>
              Total Income Wallet &nbsp;<label>{totalIncome && totalIncome.balanceIncomeWallet}</label> <span>MDR</span>
            </div>
          ...
          <div className="tableresponsive hideheader" style={{position: "relative"}}>
            ...
            {incomeWallet.length === 0 ? <div className="alertmesstable">No Records found</div> : '' }
